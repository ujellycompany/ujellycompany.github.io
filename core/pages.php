  ...
  public function throwMenu( $iType, $iPageCurrent = null, $iDepthLimit = 1, $bDisplayTitles = null ){
    ...
    if( isset( $this->mData[0] ) ){
      ...
      foreach( $this->mData[0] as $iPage => $bValue ){
        ...
        $aData['iDepth'] = 0;
        $content .= '    <li onclick="location.href=\''.$aData['sLinkName'] . '\'"'. ' class="l'.( ( !empty( $GLOBALS['config']['basket_page'] ) && $aData['iPage'] == $GLOBALS['config']['basket_page'] ) ? 'Basket' : ( ( $i == ( $iCount - 1 ) ) ? 'L': $i + 1 ) );
