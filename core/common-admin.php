...
function loginActions( $p, $sKey = 'bLogged' ){
  ...
  if( !isset( $_SESSION[$sKey] ) || $_SESSION[$sKey] !== TRUE ){
    ...
    if( isset( $iFailed ) && isset( $iFailedLoginTime ) && $iFailed > 10 && time( ) - $iFailedLoginTime <= 60 ){
      $bLoginExceed = true;
      $p = null;
    }
    ...
    if( $p == 'login' && isset( $_POST['sLogin'] ) && isset( $_POST['sPass'] ) ){
      ...
      if( $iCheckLogin == 1 ){
        ...
        $content = '<div id="error">'.$lang['Wrong_login_or_pass'].'<div id="back"><a href="javascript:history.back()">&laquo; '.$lang['back'].'</a> | <a href="http://" target="_blank">'.$lang['Forgot_your_password'].'</a></div></div>';
      }
    }
    else{
...
function listNotifications( ){
  ...
  if( is_file( 'index.php' ) && time( ) - filemtime( 'index.php' ) > 6480000 ){
    $sReturn .= '<li>'.$lang['Check_for_fixes'].' <a href="http://" target="_blank">'.$lang['More'].' &raquo;</a></li>';
  }
